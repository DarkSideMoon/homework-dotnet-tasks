...
WORKDIR /src
COPY ["Homework.Dotnet.Tasks.Image.AWSLambda.csproj", "Homework.Dotnet.Tasks.Image.AWSLambda/"]
RUN dotnet restore "Homework.Dotnet.Tasks.Image.AWSLambda/Homework.Dotnet.Tasks.Image.AWSLambda.csproj"

WORKDIR "/src/Homework.Dotnet.Tasks.Image.AWSLambda"
COPY . .
RUN dotnet build "Homework.Dotnet.Tasks.Image.AWSLambda.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "Homework.Dotnet.Tasks.Image.AWSLambda.csproj" \
...
Execute unit tests
```
cd "Homework.Dotnet.Tasks.Image.AWSLambda/test/Homework.Dotnet.Tasks.Image.AWSLambda.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "Homework.Dotnet.Tasks.Image.AWSLambda/src/Homework.Dotnet.Tasks.Image.AWSLambda"
